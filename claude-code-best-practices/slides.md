...
`@` を使用して、Claude がファイルを読むのを待たずに、直接ファイルやディレクトリを参照できます。
...
```
> Compare @file1.ts and @file2.ts for consistency
```
...
```
> Review the security in @src/middleware/auth.ts
```
...
```
> What's the structure of @src/api?
```

ファイルリスト（内容ではなく）が表示されます
...
`@server:resource` 形式でMCPサーバーからデータを取得
...
- **即座にコンテキストに追加**: Claudeが読むのを待つ必要がない
- **関連CLAUDE.mdも自動読み込み**: ファイルのディレクトリと親ディレクトリのCLAUDE.mdが追加される
- **複数ファイル可能**: 1つのメッセージで複数参照できる（例: `@file1.js and @file2.js`）
- **相対パス・絶対パス対応**: どちらでも使用可能
...
Plan Mode では、Claude は**読み取り専用操作のみ**で動作します。
...
- **マルチステップの実装**: 多くのファイル編集が必要な機能
- **コード探索**: 変更前の徹底的な調査
- **インタラクティブな開発**: Claudeと方向性を反復
...
Claude はファイルを読み取り、変更せずに質問に答えます

### 2️⃣ 計画 (Plan Mode)

```
> Google OAuth を追加したい。詳細な移行計画を作成して
```
...
**Shift+Tab** で Normal Mode に戻る

```
> プランに従って OAuth フローを実装して。
> コールバックハンドラのテストを書いて、テストスイートを実行して
```

### 4️⃣ コミット

```
> 説明的なメッセージでコミットして PR を作成
```
...
- スコープが明確で修正が小さい場合はスキップ可能
- 複雑なリファクタリングや新機能には必須
- `Ctrl+G` でプランを編集してから実装
