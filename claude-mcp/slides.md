...
uvx のパスは　`which uvx` で確認してください

---
layout: image

image: ./assets/CleanShot 2025-04-11 at 01.27.55.png
---
...
## AAWS Documentation MCP Server を使用する理由
...
# Deno を使ってMCP Server を作ってみよう

---

## Deno とは？

- Node.jsの創始者によって開発された新しいJavaScriptランタイム
...
VS Codeを使用する場合はDeno のプラグインをインストールしてDenoを有効化する  
settings.json に追加  
```json
{
  "deno.enable": true
...
```
WebStorm は標準で統合されているので準備は不要  

---

# # MCPプロジェクトを作成する

mcp-server-sample という名前でプロジェクトを作成する  
...
1. 3を倍にして
2. 118839423234 を倍にして

小さな数値だとLLMが処理できるのでMCPが使用されない  
大きな数字を入れると倍になる  
